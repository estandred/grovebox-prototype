...
async function exportSongToFile(song) {
  // Gather every sample referenced by this song's pads.
  const sampleIds = new Set();
  for (const trackId of Object.keys(song.pads || {})) {
    for (const pad of song.pads[trackId] || []) {
      if (pad?.sampleId) sampleIds.add(pad.sampleId);
    }
  }
  const samples = {};
  ...
  a.href = url;
  a.download = `${slugify(song.name) || "song"}.beatstudio.json`;
...
async function loadCloudSongs() {
  ...
  try {
    ...
    if (!res.ok) return { found: false, imported: 0, updated: 0 };
    manifest = await res.json();
  } catch {
    return { found: false, imported: 0, updated: 0 };
  }
  const files = manifest && Array.isArray(manifest.songs) ? manifest.songs : [];
  if (files.length === 0) return { found: true, imported: 0, updated: 0 };
  ...
  // Fetch in parallel; each writes through importCloudSongPayload so they
  // share the same id-based de-duplication.
...
const Audio = (() => {
  ...
  function setPumpParams(trackId, mainKnob, compAmount, volAmount, rate, intensity) {
    ...
    chain.pumpDry.gain.setTargetAtTime(1 - k, t, tc);
    chain.pumpWet.gain.setTargetAtTime(k,     t, tc);
...
function openExportModal() {
  ...
  const exportBtn = el("button", {
    ...
    onclick: async () => {
      ...
      let ok = 0;
      for (const s of list) {
        try { await exportSongToFile(s); ok++; }
        catch (err) { console.warn("export failed", s.name, err); }
      }
      toast(`exported ${ok} song${ok === 1 ? "" : "s"}`);
    },
  }, "export");
  host.replaceChildren(
...
// Modal: "create from scratch" vs "import a song". Shown when the user clicks
// "+ new song" — either path lands them in edit mode for the new song.
function openCreateOrImportModal() {
  const host = ensureHost("modal-host", "modal-host");
  const close = () => host.remove();
  const goCreate = () => {
    close();
    // Original name-prompt flow.
    promptModal({
      title: "new song",
      placeholder: "song name",
      initial: `song ${loadSongs().length + 1}`,
  ...
  host.replaceChildren(
    el("div", { class: "modal new-song-modal" },
      el("h3", {}, "new song"),
      el("p", {}, "start from scratch or load a previously exported file."),
      el("div", { class: "new-song-choices" },
        ...
        el("button", { class: "new-song-choice", onclick: goImport },
          el("strong", {}, "import a song"),
          el("span", {}, "load a .json song file exported from beat studio"),
        ),
      ),
      el("div", { class: "modal-actions" },
        el("button", {
...
// Initial render uses local storage instantly so the user doesn't stare at
// an empty screen while the cloud library loads.
render();

// Cloud library: fetch songs/manifest.json once on boot and import any
// songs published to the repo. Re-render the home/edit list if new ones
// arrived so they show up without a manual refresh.
loadCloudSongs().then((result) => {
